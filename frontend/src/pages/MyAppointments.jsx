  ...
  const initPay = (order) => {
    const options = {
      ...
      handler: function (response) {
        ...
        try {
          const {data} = axios.post(`${backendUrl}/api/user/verify-razorpay`,response,{
            headers:{token}
          })
          if(data.success){
            toast.success(data.message)
        ...
        {appointments.slice(0,3).map((item,index)=>(
          <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b'  key={index}>
            ...
            <div className='flex flex-col gap-2 justify-end'>
              {!item.cancelled && item.payment && !item.isCompleted && <p className='text-sm text-red-600 font-medium'>Paid: ₹{item.amount}</p>}
              ...
              {item.cancelled && !item.isCompleted && <button className='sm:min-w-48 p-2 border border-red-600 rounded text-red-600 text-semibold'>Appointment Cancelled</button>}
              {item.isCompleted &&  <button className='sm:min-w-48 p-2 border border-green-500 rounded text-green-500 text-semibold' >Completed</button> }
            </div>
          </div>
        ))}
      </div>
